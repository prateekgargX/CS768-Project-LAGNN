...
all_val_acc = []
all_test_acc = []
for i in trange(args.runs, desc='Run Experiments', disable=True):
    # Model and optimizer
...
print(f"val_mu: {np.mean(all_val_acc):.4f}, val_std: {np.std(all_val_acc):.2e}, test_mu: {np.mean(all_test_acc):.4f}, test_std: {np.std(all_test_acc):.2e}")
